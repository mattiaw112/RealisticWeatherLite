-------------------------------------------------------------------------------
-- REALISTIC WEATHER LITE (Aggiornato - Fix Nebbia Base)
-------------------------------------------------------------------------------
...
function RealisticWeatherLite:update(dT)
    ...
    ---------------------------------------------------------------------------
    -- 3. GESTIONE NEBBIA DINAMICA (Modificata per non bloccare la base)
    ---------------------------------------------------------------------------
    ...
    -- APPLICA LA NEBBIA SOLO SE LA MOD LA STA GENERANDO ATTIVAMENTE, ALTRIMENTI LASCIA LIBERO IL GIOCO BASE
    if RealisticWeatherLite.currentFogDensity > 0.01 then
        ...
        RealisticWeatherLite.hasWarnedFog = false
        -- RIMOSSO IL RESET A ZERO FORZATO: in questo modo il gioco base gestisce la sua nebbia liberamente
    end
end
...
-------------------------------------------------------------------------------
-- GESTIONE RANDOMIZZAZIONE (Non interferisce con i preset nativi)
-------------------------------------------------------------------------------
function RealisticWeatherLite.randomizeFog(self, time)
    -- Lasciamo che la funzione nativa del gioco gestisca la randomizzazione standard senza blocchi aggressivi
end

if Weather ~= nil and Weather.randomizeFog ~= nil then
    -- Rimuoviamo il blocco distruttivo sulla randomizzazione base
end